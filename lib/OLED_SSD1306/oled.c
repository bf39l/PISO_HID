...
// OLED器件地址
#define OLED_SSD1306_ADDR 0x3C              // 7-bit address
#define OLED_ADDRESS      OLED_SSD1306_ADDR // (OLED_SSD1306_ADDR << 1) // STM32 HAL expands to 0x78
...
{
  i2c_write_blocking(i2c_instance, OLED_ADDRESS, data, len, true);
}
...
void OLED_ShowFrame()
{
  ...
  for (uint8_t i = 0; i < OLED_PAGE; i++)
  {
    ...
    OLED_SendCmd(0x00 + columnOffset); // 设置列地址低4位
